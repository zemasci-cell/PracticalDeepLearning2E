#
#  file:  resnet18_pooling.py
#
#  ResNet-18 applied to CIFAR-10 using pooling layers
#  in place of (2,2) strides
#
#  RTK, 17-Aug-2023
...
def ResidualBlock(x, filters, downsample=False, useBN=False, useMax=False):
    """Build a residual block"""
    if (downsample):
        _ = Conv2D(filters, (1,1), strides=(1,1), padding='same')(x)
        if (useMax):
            inp = MaxPooling2D((2,2))(_)
        else:
            inp = AveragePooling2D((2,2))(_)
    else:
        inp = x

    _ = Conv2D(filters, (3,3), strides=(1,1), padding='same')(x)
    if (useBN):
...
useMax = True if (int(sys.argv[4])) else False
outdir = sys.argv[5]
...
_ = ResidualBlock(_,  64, downsample=False, useBN=useBN, useMax=useMax)
_ = ResidualBlock(_,  64, downsample=False, useBN=useBN, useMax=useMax)
_ = ResidualBlock(_, 128, downsample=True,  useBN=useBN, useMax=useMax)
_ = ResidualBlock(_, 128, downsample=False, useBN=useBN, useMax=useMax)
_ = ResidualBlock(_, 256, downsample=True,  useBN=useBN, useMax=useMax)
_ = ResidualBlock(_, 256, downsample=False, useBN=useBN, useMax=useMax)
_ = ResidualBlock(_, 512, downsample=True,  useBN=useBN, useMax=useMax)
_ = ResidualBlock(_, 512, downsample=False, useBN=useBN, useMax=useMax)
