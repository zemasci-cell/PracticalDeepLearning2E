#
#  file:  interpolate_random_mosaic.py
#
#  Use a trained generator model to interpolate images
#  along a randomly selected line in the latent space
#
#  RTK, 10-Apr-2023
...
# seeds 12345, 31415*, 8675309, 8080
...
if (len(sys.argv) == 1):
    print()
    print("interpolate_random_mosaic <model> <latent> <out_image> [<seed>]")
    print()
...
for d in range(10):
    ...
    slopes = 1.5*(np.random.random(size=latent_dim)-0.5)

    tmin,tmax = [], []
    for i in range(latent_dim):
        if (slopes[i] > 0):
            tmin.append((lo - base[0,i]) / slopes[i])
            tmax.append((hi - base[0,i]) / slopes[i])
        else:
            tmax.append((lo - base[0,i]) / slopes[i])
            tmin.append((hi - base[0,i]) / slopes[i])
    tmn,tmx = max(tmin), min(tmax)
    
    for i,x in enumerate(np.linspace(tmn,tmx,16)):
        noise = base + x * slopes
